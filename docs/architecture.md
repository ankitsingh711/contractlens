...
> Status: reflects Phase 1–4 (auth, document pipeline, hybrid RAG, LangGraph agent). Diagrams for later phases (evaluation harness, observability, production cloud deploy) are marked **target state**. See the roadmap in the root [README](../README.md).
...
- **`apps/web`** — Next.js 15 (App Router, TypeScript, Tailwind, shadcn/ui, TanStack Query). Renders the dashboard, documents, AI assistant, agent-run trace viewer, evaluations, and settings screens.
...
```
apps/api/app/
├── api/v1/        # HTTP layer: routers (auth, documents, search, chat, conversations, agent_runs, health)
...
├── services/      # business logic: auth, document_service, rag_service, citations, embeddings, reranking, llm
...
## 10. Cloud architecture — target state (Phase 8)
